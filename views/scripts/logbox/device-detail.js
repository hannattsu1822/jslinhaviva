  ...
  try {
    ...
    if (latestReadingResponse.ok) {
      ...
      const tempExterna =
        ...
        (payloadObjeto.value_channels
          ? payloadObjeto.value_channels[2]
          : undefined);

      const latestTempEl = document.getElementById("latest-temp");
      if (latestTempEl && tempExterna !== undefined) {
...
function atualizarListaAlarmes(alarms) {
  ...
  if (Array.isArray(alarms)) {
    ...
    activeAlarms = Object.entries(alarms).filter(
...
async function carregarEstatisticasDetalhes() {
  try {
    ...
    if (statTodayMin) statTodayMin.textContent = stats.today.min;
    const statTodayAvg = document.getElementById("stat-today-avg");
    if (statTodayAvg) statTodayAvg.textContent = stats.today.avg;
    const statTodayMax = document.getElementById("stat-today-max");
    if (statTodayMax) statTodayMax.textContent = stats.today.max;

    const statMonthMin = document.getElementById("stat-month-min");
    if (statMonthMin) statMonthMin.textContent = stats.month.min;
    const statMonthAvg = document.getElementById("stat-month-avg");
    if (statMonthAvg) statMonthAvg.textContent = stats.month.avg;
    const statMonthMax = document.getElementById("stat-month-max");
    if (statMonthMax) statMonthMax.textContent = stats.month.max;
  } catch (error) {
...
async function carregarDadosVentilacao() {
  try {
    ...
    if (!historyResponse.ok) throw new Error("Falha ao buscar histórico de ventilação");
...
async function carregarHistoricoConexao() {
  try {
    ...
    const totalDisconnectsEl = document.getElementById("conn-total-disconnects");
    if (totalDisconnectsEl) {
      totalDisconnectsEl.textContent = data.summary.total_disconnects;
    }

    const avgDurationEl = document.getElementById("conn-avg-duration");
    if (avgDurationEl) {
      avgDurationEl.textContent = formatarDuracao(
        data.summary.avg_duration_seconds
      );
    }
    ...
    if (data.history.length === 0) {
      tableBody.innerHTML =
        '<tr><td colspan="4" class="text-center text-muted">Nenhum evento de desconexão registrado.</td></tr>';
    } else {
      data.history.forEach((item) => {
        const row = tableBody.insertRow();
        row.insertCell(0).textContent = new Date(
          item.timestamp_offline
        ).toLocaleString("pt-BR");
        row.insertCell(1).textContent = item.timestamp_online
          ? new Date(item.timestamp_online).toLocaleString("pt-BR")
          : "Ainda Offline";
        row.insertCell(2).textContent = formatarDuracao(item.duracao_segundos);
        row.insertCell(3).textContent = item.ultimo_rssi
          ? `${item.ultimo_rssi} dBm`
          : "N/A";
      });
    }
  } catch (error) {
