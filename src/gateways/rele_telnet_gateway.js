...
async function pollRele(rele) {
  ...
  const params = {
    ...
    port: rele.port,
    shellPrompt: '=>', // O prompt que indica que o relé está pronto
    loginPrompt: 'User>', // O prompt de usuário
    passwordPrompt: 'Password>', // O prompt de senha
    username: 'ACC',
    password: 'OTTER',
  ...
  try {
    ...
    const response = await connection.exec('MET'); // A biblioteca espera pelo '=>' automaticamente
    ...
    if (parsedData) {
      ...
      const sql = `INSERT INTO leituras_reles (rele_id, timestamp_leitura, tensao_a, tensao_b, tensao_c, corrente_a, corrente_b, corrente_c, frequencia, payload_completo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
      await promisePool.query(sql, [rele.id, timestamp, tensao_a, tensao_b, tensao_c, corrente_a, corrente_b, corrente_c, frequencia, response]);
      console.log(`[Telnet Gateway] [${rele.local_tag}] SUCESSO: Dados salvos no banco.`);

      // Lógica para publicar no MQTT (opcional, mas bom para o frontend)
      const topic = `sel/reles/${rele.id}/status`;
      const payload = JSON.stringify({ rele_id: rele.id, local_tag: rele.local_tag, ...parsedData, timestamp_leitura: timestamp });
