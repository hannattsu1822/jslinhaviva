...
} = require("../init"); // Adicionado uploadsSubestacoesDir
...
async function verificarInspecaoExiste(req, res, next) {
  ...
  try {
    ...
    req.inspecao = inspecao[0]; // Salvar a inspeção para uso na rota de exclusão
...
router.post(
  ...
  async (req, res) => {
    ...
    let dataInstalacao = date_inst ? date_inst : null;
    if (dataInstalacao && !/^\d{4}-\d{2}-\d{2}$/.test(dataInstalacao)) {
      try {
        dataInstalacao = new Date(dataInstalacao).toISOString().split("T")[0];
        if (!/^\d{4}-\d{2}-\d{2}$/.test(dataInstalacao)) throw new Error();
      } catch (e) {
        return res
          .status(400)
  ...
  async (req, res) => {
    ...
    if (date_inst !== undefined) {
      if (date_inst === null || String(date_inst).trim() === "") {
        ...
        try {
          dataInstalacao = new Date(date_inst).toISOString().split("T")[0];
          if (!/^\d{4}-\d{2}-\d{2}$/.test(dataInstalacao)) throw new Error();
        } catch (e) {
          return res
            .status(400)
    ...
    try {
      ...
      if (arquivos && arquivos.length > 0) {
        ...
        for (const file of arquivos) {
          ...
          const caminhoRelativoServidor = `/upload_arquivos_subestacoes/servicos/servico_${novoServicoId}/${nomeUnicoArquivo}`;
          ...
          anexosSalvosInfo.push({
            ...
            caminho_servidor: caminhoRelativoServidor,
          });
        }
      }
    ...
    try {
      ...
      if (arquivosNovos && arquivosNovos.length > 0) {
        ...
        for (const file of arquivosNovos) {
          ...
          const caminhoDestino = path.join(servicoUploadDir, nomeUnicoArquivo);
          const caminhoRelativoServidor = `/upload_arquivos_subestacoes/servicos/servico_${servicoId}/${nomeUnicoArquivo}`;
          await fs.rename(file.path, caminhoDestino);
          arquivosMovidosComSucesso.push(caminhoDestino);
          const [resultAnexo] = await connection.query(
            `INSERT INTO Servicos_Subestacoes_Anexos (id_servico, nome_original, caminho_servidor, tipo_mime, tamanho, categoria_anexo) VALUES (?, ?, ?, ?, ?, ?)`,
      ...
      if (anexos.length > 0) {
        ...
        for (const anexo of anexos) {
          const caminhoCompleto = path.join(
            projectRootDir,
            anexo.caminho_servidor.startsWith("/")
              ? anexo.caminho_servidor.substring(1)
        ...
        try {
          await fs.rmdir(servicoUploadDir);
        } catch (err) {
    ...
    try {
      ...
      if (arquivosConclusao && arquivosConclusao.length > 0) {
        ...
        for (const file of arquivosConclusao) {
          ...
          const caminhoDestino = path.join(servicoUploadDir, nomeUnicoArquivo);
          const caminhoRelativoServidor = `/upload_arquivos_subestacoes/servicos/servico_${servicoId}/conclusao/${nomeUnicoArquivo}`;
          await fs.rename(file.path, caminhoDestino);
          arquivosMovidosComSucesso.push(caminhoDestino);
          const [resultAnexo] = await connection.query(
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      let anexosSalvosInfo = [];
      const baseUploadDirRoot = path.join(
        projectRootDir,
        "upload_arquivos_subestacoes"
      );
      ...
      for (const file of generalAttachments) {
        ...
        const caminhoRelativoServidor = `/upload_arquivos_subestacoes/checklist/checklist_${novaInspecaoId}/${nomeUnicoArquivo}`;
        await fs.rename(file.path, caminhoDestino);
  ...
  async (req, res) => {
    ...
    try {
      ...
      for (const file of arquivos) {
        ...
        const caminhoRelativoServidor = `/upload_arquivos_subestacoes/inspecoes_escritorio/inspecao_escritorio_${inspecaoId}/${nomeUnicoArquivo}`;
        await fs.rename(file.path, caminhoDestino);
        arquivosMovidosComSucesso.push(caminhoDestino);
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (anexos.length > 0) {
        const anexoBaseDir = path.join(
          uploadsSubestacoesDir,
          "checklist",
          `checklist_${String(inspecaoId)}`
        );
        const escritorioBaseDir = path.join(
          uploadsSubestacoesDir,
          "inspecoes_escritorio",
          `inspecao_escritorio_${String(inspecaoId)}`
        );
        ...
        try {
          await fs.rm(anexoBaseDir, { recursive: true, force: true });
        } catch (e) {
          console.warn(`Falha ao remover dir ${anexoBaseDir}: ${e.message}`);
        }
        try {
          await fs.rm(escritorioBaseDir, { recursive: true, force: true });
        } catch (e) {
          console.warn(
            `Falha ao remover dir ${escritorioBaseDir}: ${e.message}`
          );
        }
      }
