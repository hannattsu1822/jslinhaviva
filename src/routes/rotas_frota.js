...
router.get("/api/placas", autenticar, verificarNivel(2), async (req, res) => {
  try {
    const [rows] = await promisePool.query(
      `SELECT
        v.id,
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Erro ao buscar veículos do inventário!" });
    }
  }
);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (existing.length > 0) {
        return res
          .status(409)
          .json({ message: "Código da peça já cadastrado." });
      }
