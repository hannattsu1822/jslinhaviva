...
function podeAcessarModulo() {
  return P().podeAcessarModuloConstrucao?.(user) ?? false;
}
...
async function carregarDetalhesServico() {
  ...
  try {
    const response = await fetch(`/api/construcao/servicos/${servicoId}`, {
    ...
    if (data.status === "concluido" || data.status === "nao_concluido") {
      if (dataConclusaoRow) dataConclusaoRow.style.display = "block";
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!podeAcessarModulo()) {
    window.location.replace("/login");
    return;
  }

  const accessDeniedEl = document.getElementById("access-denied-modal");
  if (accessDeniedEl && typeof bootstrap !== "undefined" && bootstrap.Modal) {
    accessDeniedModalInstance = new bootstrap.Modal(accessDeniedEl);
  }
  ...
  if (btnPdf && servicoId) {
    btnPdf.addEventListener("click", () => {
      window.open(`/api/construcao/servicos/${servicoId}/pdf`, "_blank");
    });
  }
});
