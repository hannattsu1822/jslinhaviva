...
function renderizarListaEncarregados(lista) {
    ...
    lista.forEach((enc) => {
        ...
        div.innerHTML = `
            ...
            <button class="btn btn-sm ${selecionado ? "btn-danger" : "btn-success"}" onclick="toggleResponsavel('${enc.matricula}')">
...
function renderizarPreviewFotos() {
    ...
    selectedFiles.forEach((f, i) => {
        ...
        div.innerHTML = `
            <img src="${URL.createObjectURL(f)}" class="img-thumbnail" style="width: 100px; height: 100px; object-fit: cover;">
            <button type="button" class="btn btn-sm btn-danger position-absolute top-0 end-0" onclick="removerFoto(${i})">&times;</button>
        `;
        container.appendChild(div);
    });
}
