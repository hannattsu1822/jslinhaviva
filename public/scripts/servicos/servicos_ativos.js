...
let user = null;
let selectedFiles = [];         // arquivos de foto para conclusão
...
async function carregarServicosAtivos() {
  try {
    ...
    if (tbody) tbody.innerHTML = `<tr><td colspan="8" class="text-center py-4">Falha ao carregar serviços. Tente atualizar.</td></tr>`;
  }
}
...
function atualizarTabela() {
  ...
  let servicosFiltrados = servicosData.filter(servico => {
    ...
    const encarregadoMatch = !filtroEncarregado ||
      (servico.nomes_responsaveis && servico.nomes_responsaveis.toLowerCase().includes(filtroEncarregado.toLowerCase()));
  ...
  if (servicosFiltrados.length === 0) {
    tbody.innerHTML = `<tr><td colspan="8" class="text-center py-4">Nenhum serviço ativo encontrado.</td></tr>`;
    return;
  }
  ...
  servicosFiltrados.forEach(servico => {
    ...
    const btnConcluir = nivel5Plus
      ? `<button class="btn btn-sm btn-success w-100 mb-1" onclick="abrirConcluirNivel5(${servico.id})"><span class="material-symbols-outlined" style="font-size:16px">check_circle</span> Concluir</button>`
...
function abrirConcluirNivel5(servicoId) {
  ...
  if (infoEl && servico) {
    const dataPrevista = servico.data_prevista_execucao
      ? new Date(servico.data_prevista_execucao + "T00:00:00").toLocaleDateString("pt-BR")
      : "—";
    infoEl.innerHTML = `<div class="alert alert-info py-2 mb-3"><strong>Processo:</strong> ${servico.processo || "—"} &nbsp;|&nbsp; <strong>Subestação:</strong> ${servico.subestacao || "—"} &nbsp;|&nbsp; <strong>Alimentador:</strong> ${servico.alimentador || "—"} &nbsp;|&nbsp; <strong>Data Prevista:</strong> ${dataPrevista}</div>`;
  }
...
function filtrarResponsaveis(termo) {
  ...
  const filtrados = todosEncarregados.filter(e =>
    e.nome.toLowerCase().includes(termoLower) || String(e.matricula).includes(termoLower)
  );
...
function renderizarListaEncarregados(lista) {
  ...
  lista.forEach(enc => {
    ...
    div.innerHTML = `
      <div><strong>${enc.nome}</strong><br><small class="text-muted">Mat: ${enc.matricula} — ${enc.cargo || ""}</small></div>
      <button class="btn btn-sm ${selecionado ? "btn-danger" : "btn-success"}" onclick="toggleResponsavel('${enc.matricula}')">${selecionado ? "Remover" : "Adicionar"}</button>
    `;
