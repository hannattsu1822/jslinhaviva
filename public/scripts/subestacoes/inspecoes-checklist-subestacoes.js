document.addEventListener("DOMContentLoaded", () => {
  ...
  const modalJustificativa = document.getElementById(
  ...
  const btnCancelarJustificativa = document.getElementById(
    "btnCancelarJustificativa"
  );

  let closeModalJustificativaButton;
  if (modalJustificativa) {
    closeModalJustificativaButton =
      modalJustificativa.querySelector(".close-modal");
  }
  ...
  function gerarItensChecklist() {
    ...
    checklistTemplate.forEach((grupo) => {
      ...
      grupoHeader.className = "checklist-grupo-header";
      grupoHeader.innerHTML = `<h3><span class="material-icons-outlined">${
      ...
      grupo.itens.forEach((item) => {
        ...
        const itemNumeroSpan = document.createElement("span");
        itemNumeroSpan.className = "item-numero";
        itemNumeroSpan.textContent = `${item.num}.`;
        const itemDescricaoElement = document.createElement("div");
        itemDescricaoElement.className = "item-descricao";
        itemDescricaoElement.textContent = item.desc;
        const itemAvaliacaoDiv = document.createElement("div");
        itemAvaliacaoDiv.className = "item-avaliacao";
        itemAvaliacaoDiv.innerHTML = `<label><input type="radio" name="item_${item.num}_avaliacao" value="N" required> <span class="radio-label-text">N</span></label><label><input type="radio" name="item_${item.num}_avaliacao" value="A"> <span class="radio-label-text">A</span></label><label><input type="radio" name="item_${item.num}_avaliacao" value="NA"> <span class="radio-label-text">NA</span></label>`;
        itemDiv.appendChild(itemNumeroSpan);
        itemDiv.appendChild(itemDescricaoElement);
        itemDiv.appendChild(itemAvaliacaoDiv);
        const radios = itemDiv.querySelectorAll(
          `input[name="item_${item.num}_avaliacao"]`
        );
        radios.forEach((radio) => {
          radio.addEventListener("change", (event) => {
            handleAvaliacaoChange(event, itemDiv, itemDescricaoElement);
          });
        });
        grupoSection.appendChild(itemDiv);
      });
  ...
  function handleAvaliacaoChange(event, itemElement, itemDescElement) {
    ...
    const labels = itemElement.querySelectorAll(".item-avaliacao label");
    labels.forEach((label) => {
      label.classList.remove("selected-n", "selected-a", "selected-na");
    });
    if (event.target.closest("label")) {
      event.target
        .closest("label")
        .classList.add(`selected-${selectedValue.toLowerCase()}`);
    }
    if (selectedValue === "A") {
      ...
      if (currentFile) {
        if (fotoAnormalidadeNomeP) fotoAnormalidadeNomeP.textContent = "";
      ...
      if (modalJustificativa) {
        mostrarModal(modalJustificativa);
        if (justificativaTextarea) {
          setTimeout(() => {
    ...
    if (modalFileInputWrapper) {
      ...
      modalFileInputWrapper.addEventListener("drop", (event) => {
        ...
        if (event.dataTransfer.files && event.dataTransfer.files.length > 0) {
          ...
          fotoAnormalidadeInput.dispatchEvent(changeEvent);
        }
      });
    }
  }
  ...
  if (btnDeleteFotoAnormalidade) {
    btnDeleteFotoAnormalidade.addEventListener("click", () => {
      if (currentItemAnormalElement) {
        ...
        delete itemAnormalData[`item_${itemNum}_fotoFile`];
        if (fotoAnormalidadeInput) fotoAnormalidadeInput.value = null;
        if (fotoAnormalidadePreview) fotoAnormalidadePreview.src = "#";
        if (fotoAnormalidadePreview)
          fotoAnormalidadePreview.style.display = "none";
        btnDeleteFotoAnormalidade.style.display = "none";
        if (fotoAnormalidadePreviewContainer)
  ...
  if (modalJustificativa) {
    if (closeModalJustificativaButton) {
      closeModalJustificativaButton.addEventListener("click", () =>
        ocultarModal(modalJustificativa)
      );
    }
    modalJustificativa.addEventListener("click", (event) => {
  ...
  if (btnCancelarJustificativa) {
    btnCancelarJustificativa.addEventListener("click", () => {
      if (modalJustificativa) ocultarModal(modalJustificativa);
    });
  }
  if (btnSalvarJustificativa) {
    btnSalvarJustificativa.addEventListener("click", () => {
      if (
        ...
        if (
          !justificativa &&
          currentItemAnormalElement.querySelector('input[value="A"]:checked')
        ) {
        ...
        if (
          ...
          fotoAnormalidadeInput.focus();
          return;
        }
        itemAnormalData[`item_${itemNum}_obs`] = justificativa;
        if (descElement) {
          if (justificativa) {
            descElement.classList.add("anormal-justificada");
          } else {
            descElement.classList.remove("anormal-justificada");
          }
          if (fotoFile) {
            descElement.classList.add("anormal-com-foto");
          } else {
            descElement.classList.remove("anormal-com-foto");
          }
        }
        if (modalJustificativa) ocultarModal(modalJustificativa);
      }
  ...
  function renderGeneralAttachmentsList() {
    ...
    selectedGeneralFiles.forEach((file, index) => {
      ...
      const previewElement = document.createElement(
        file.type.startsWith("image/") ? "img" : "div"
      );
      previewElement.className = file.type.startsWith("image/")
        ? "file-preview"
        : "file-preview-icon";
      if (file.type.startsWith("image/")) {
        const reader = new FileReader();
        reader.onload = (e) => {
          ...
          previewElement.alt = `Pré-visualização de ${file.name}`;
        };
        reader.readAsDataURL(file);
      } else {
      ...
      fileInfoSpan.innerHTML = ` <span class="file-name">${
        file.name
      }</span> <span class="file-size">(${(file.size / 1024).toFixed(
        1
      )} KB)</span> `;
      li.appendChild(fileInfoSpan);
      const deleteButton = document.createElement("button");
      deleteButton.type = "button";
      deleteButton.className = "btn-delete-anexo";
    ...
    if (wrapper) {
      ...
      wrapper.addEventListener("drop", (event) => {
        ...
        if (event.dataTransfer.files && event.dataTransfer.files.length > 0) {
          ...
          const changeEvent = new Event("change", { bubbles: true });
          inspecaoAnexosInput.dispatchEvent(changeEvent);
        }
      });
    }
  }
  ...
  if (formChecklistInspecao) {
    formChecklistInspecao.addEventListener("submit", async (event) => {
      ...
      document.querySelectorAll(".checklist-item").forEach((itemDiv) => {
        ...
        if (avaliacao === "A") {
          ...
          if (!fotoFile || !obs) {
            ...
            alert(
              `Item ${itemNum} (${itemDiv.getAttribute(
                "data-item-desc"
              )}) está marcado como Anormal, mas falta a justificativa ou a foto de evidência. Por favor, complete no modal.`
            );
      ...
      if (!todosItensAnormaisCompletos) {
        alert(
          "Existem itens anormais sem justificativa e/ou foto. Verifique os itens destacados."
        );
        return;
      }
      ...
      try {
        ...
        btnSalvarInspecao.innerHTML =
          '<span class="material-icons-outlined">save</span> Salvar Inspeção';
