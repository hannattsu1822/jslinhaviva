import { showLoading, hideLoading, showError, showMessage, parseHTMLDescription } from './mapa-ui.js';
...
class MapaDinamico {
    ...
    async carregarPlugins() {
        ...
        await Promise.all(plugins.map(url => {
    ...
    async carregarArquivosData() {
        try {
            ...
            if (!arquivos.length) throw new Error('Nenhum arquivo encontrado na pasta data');
            
            showMessage(`Encontrados ${arquivos.length} arquivos na pasta data`, 'info');
    ...
    processarDadosArquivo(data, nomeArquivo) {
        ...
        data.features.forEach(feature => {
            if (feature.geometry?.type === 'Point') {
                ...
                const marker = this.criarMarcador(feature, tipoNormalizado, nomeArquivo);
    ...
    criarMarcador(feature, tipo, nomeArquivo) {
        ...
        const iconePersonalizado = this.criarIconePersonalizado(configIcone.cor, configIcone.forma);
    ...
    criarInterfaceFiltros() {
        // Atualiza contadores
        document.getElementById('contador-arquivos').textContent = this.arquivosCarregados.length;
        document.getElementById('contador-tipos').textContent = this.tiposDisponiveis.size;
        document.getElementById('contador-ativos').textContent = this.arquivosCarregados.reduce((acc, arquivo) => acc + arquivo.quantidade, 0);
        ...
        document.getElementById('btn-resetar-filtros').addEventListener('click', () => this.resetarFiltros());
    }

    aplicarFiltros() {
        // Lógica para aplicar filtros selecionados
        const arquivosAtivos = Array.from(document.querySelectorAll('#lista-arquivos .form-check-input:checked')).map(el => el.id.replace('arquivo-', ''));
        const tiposAtivos = Array.from(document.querySelectorAll('#lista-tipos .form-check-input:checked')).map(el => el.id.replace('tipo-', ''));
        ...
        showMessage(`Filtros aplicados: ${arquivosAtivos.length} arquivos e ${tiposAtivos.length} tipos visíveis`, 'success');
    }

    resetarFiltros() {
        document.querySelectorAll('#lista-arquivos .form-check-input').forEach(el => el.checked = true);
        document.querySelectorAll('#lista-tipos .form-check-input').forEach(el => el.checked = true);
        this.aplicarFiltros();
    }
    ...
    criarLegenda() {
        ...
        const tiposLegenda = ['poste', 'seccionadora', 'trafo'];
    ...
    getSvgShape(forma, cor) {
        switch(forma) {
            case 'circle': return `<circle cx="12" cy="12" r="8" fill="${cor}"/>`;
            case 'square': return `<rect x="6" y="6" width="12" height="12" fill="${cor}"/>`;
            case 'triangle': return `<path d="M12 4 L20 18 L4 18 Z" fill="${cor}"/>`;
            default: return `<circle cx="12" cy="12" r="8" fill="${cor}"/>`;
        }
    }
    ...
    filtrarAtivosDentroDePoligono(polygon) {
        ...
        showMessage(`Encontrados ${ativosFiltrados} ativos na área selecionada`, 'info');
    }
    ...
    mostrarDetalhesAtivo(ativo) {
        ...
        modalTitulo.innerHTML = `<i class="bi bi-info-circle"></i> Detalhes do ${this.formatarTipoLegenda(ativo.tipo)}`;
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <table class="table table-sm">
                        <tr><th>Código</th><td>${ativo.codigo || 'N/A'}</td></tr>
                        <tr><th>Tipo</th><td>${this.formatarTipoLegenda(ativo.tipo)}</td></tr>
                ...
                <div class="col-md-6">
                    ...
                    <table class="table table-sm">
                        <tr><th>Latitude</th><td>${ativo.geometry.coordinates[1].toFixed(6)}</td></tr>
                        <tr><th>Longitude</th><td>${ativo.geometry.coordinates[0].toFixed(6)}</td></tr>
                    </table>
                </div>
            </div>
        `;
